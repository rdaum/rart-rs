...
impl<N> MultilevelNode4<N> {
    ...
    /// Extract the single child when this node has only one child (for shrinking)
    pub fn take_value_for_leaf(&mut self) -> (Vec<u8>, N) {
        debug_assert_eq!(self.num_children, 1);

        // Extract the key bytes
        let key_len = self.key_lengths[0] as usize;
        let key_bytes = self.keys[0][..key_len].to_vec();
        ...
        // Reset the node
        self.keys[0] = [0; 4];
        self.key_lengths[0] = 0;
        self.num_children = 0;

        (key_bytes, unsafe { child.assume_init() })
    }
}
...
impl<N> NodeMapping<N, 4> for MultilevelNode4<N> {
    ...
    fn add_child(&mut self, key: u8, node: N) {
        ...
        self.add_multilevel_child(&[key], node)
            .expect("Node should not be full");
    }
        ...
        for i in 0..self.num_children as usize {
            if self.key_lengths[i] > 0 && self.keys[i][0] == key {
                ...
                for j in i..(self.num_children as usize - 1) {
                    ...
                    self.children[j] =
                        std::mem::replace(&mut self.children[j + 1], MaybeUninit::uninit());
                }
